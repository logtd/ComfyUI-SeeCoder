...
from .configs import swin_large_config, seecoder_decoder_config, seecoder_query_config, seecoder_query_pa_config
...
class SemanticContextEncoder(nn.Module):
    def __init__(self, has_pe=False):
        super().__init__()
        query_config = seecoder_query_config if not has_pe else seecoder_query_pa_config
        self.imencoder = SwinTransformer(**swin_large_config)
        self.imdecoder = Decoder(**seecoder_decoder_config)
        self.qtransformer = QueryTransformer(**query_config)
